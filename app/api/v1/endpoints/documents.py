...
async def upload_document(session_id: str, file: UploadFile = File(...)) -> DocumentUploadResponse:
    ...
    return DocumentUploadResponse(
        doc_id=doc_id, filename=filename, chunks=len(chunks), session_id=session_id
    )
...
async def list_documents(session_id: str) -> DocumentListResponse:
    ...
    return DocumentListResponse(
        session_id=session_id, doc_ids=session.get("doc_ids", [])
    )
